...
def render_aggregate_row(
    ...
    avg_cost_val = f"{currency_symbol}{avg_cost_per_invoice:,.2f}" if avg_cost_per_invoice is not None else "—"
    total_cost_val = f"{currency_symbol}{total_cost:,.2f}" if total_cost is not None else "—"
