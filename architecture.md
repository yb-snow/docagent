...
| **`fx_convert`** | If enabled (`config.FX_CONVERSION_ENABLED`, toggle in Settings): detects the document's currency and, if not `config.LOCAL_CURRENCY`, fetches a live rate from a free FX API and converts every monetary field + line item. If unknown, assumes local currency. If the rate fetch fails, flags the document for mandatory human review. If disabled, the step is a no-op — documents keep their original currency and are never routed to review on currency grounds. | Human review if FX fetch fails (only when enabled) |
...
Detects currency from the `currency`/`currency_code` field or by scanning for `$`/`€`/`£` symbols in values. If the document's currency differs from `config.LOCAL_CURRENCY` (configurable in Settings → Currency, default `INR`), calls the free Frankfurter FX API (no key required) and converts every field matching a monetary-keyword regex (`total|amount|subtotal|tax|vat|gst|...`) plus every line item. If the API call fails, the document is **not** silently left in the wrong currency — it's explicitly flagged for mandatory human review (`fx_review_required`), which the router checks before anything else. This is a good example of "fail loud, not silent" design in a financial-data context.

**`config.FX_CONVERSION_ENABLED`** (Settings → Currency, default on) is an explicit escape hatch for exactly this failure mode: if the FX API is unreachable from your deployment environment (e.g. an outbound network restriction on the hosting platform), every foreign-currency document would otherwise be forced to human review regardless of extraction confidence. Turning conversion off entirely stores documents in their original currency and removes currency from the auto-approve decision — a deliberate tradeoff between "always correct currency" and "don't let an external API's uptime gate your automation rate."
